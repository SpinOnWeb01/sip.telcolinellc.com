...
function AdminExtension({ colorThem }) {
  ...
  const columns = [
   ...
   
    {
      ...
      renderCell: (params) => (<>
        <input
        className="dark-mode-color"
            type={showPasswordMap[params.id] ? 'text' : 'password'}
            value={params.row.password}
            readOnly
            style={{ border: 'none', width: '100%',color:"#000 !important" }}
        />
        <IconButton edge="end" onClick={() => handleShowPassword(params.id)} style={{fontSize:"20px"}}>
          {token.user_role === "Reseller" ? (<></>) : (<>{showPasswordMap[params.id] ? <FaEye /> : <FaEyeSlash />}</>)}
...
return <MenuItem key={index} value={item}>{item}</MenuItem>
                                    ...
                                    <InputLabel style={{textAlign:'left', fontSize: '12px',display:'flex',alignItems:'center'}}>
                                      <Tooltip style={{color:'#fff'}} title="By choosing a username, the available extensions continue to expand." classes={{ tooltip: classes.tooltip }}>
                                        <InfoIcon style={{fontSize:"18px",color:"#603e21"}} />&nbsp;
                                        </Tooltip >By choosing a username, the available extensions continue to expand.</InputLabel>
